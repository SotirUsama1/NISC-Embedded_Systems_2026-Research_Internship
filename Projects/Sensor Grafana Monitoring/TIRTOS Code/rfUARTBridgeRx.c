...
#define NUM_APPENDED_BYTES                                                     \
  3 /* header byte handled by RFQueue; RSSI + status appended by radio */
#define NO_PACKET 0
...
void *mainThread(void *arg0) {
  ...
  RF_postCmd(rfHandle, (RF_Op *)&RF_cmdPropRx, RF_PriorityNormal,
             &ReceivedOnRFcallback, RF_EventRxEntryDone);
  ...
  while (1) {
    if (packetRxCb) {
      ...
      if (packetLength == MODBUS_RX_LEN) {
        ...
        uint16_t rxCRC =
            packet[MODBUS_RX_LEN - 2] | (packet[MODBUS_RX_LEN - 1] << 8);
...
/* Must match the sender's byte order exactly (big-endian wire -> little-endian
 * ARM) */
static float bytesToFloat(const uint8_t *b) {
